...
/*   Updated: 2022/09/07 16:53:24 by jjuntune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	calculate_a(t_ray *ray, double m, t_vector	h)
{
	...
	temp2 = (m * m_a_vector(multiply_vectors(ray->dir, h),
				multiply_vectors(ray->dir, h)));
	temp3 = m_a_vector(multiply_vectors(ray->dir, h),
			multiply_vectors(ray->dir, h));
...
double	cone_intersection(t_ray *ray, t_rtv *rtv, int count)
{
	...
	h = divide_vect_float(h, sqrt((m_a_vector(h, h))));
